...
module.exports = (router) => {
  // 【主页】
  router.get('/api/homepage', (ctx, next) => {
    ctx.body = Util.readJson(path.resolve(__dirname, '../mock/api/homePage.json'));
  });

  // 【注册】将注册信息发给服务端，返回成功，成功提示，注册成功，失败，失败原因，用户名已注册
  router.get('/api/rank', async (ctx, next) => {
    let rankList = Util.readJson(path.resolve(__dirname, '../mock/api/rank.json'));
